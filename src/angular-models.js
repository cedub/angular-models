angular.module( 'lk.rest', [])
  .factory('LKBaseUrl', [function () {
    return {
      path: '/rest/'
    };
  }])
  .factory('LKRestCollection', ['LKRestModel', 'LKBaseUrl', function (LKRestModel, LKBaseUrl) {
    ...
    _.extend(RestCollection.prototype, {
      ...
      set: function(models, options) {
        ...
        for (i = 0, l = models.length; i < l; i++) {
          attrs = models[i] || {};
          if (attrs instanceof LKRestModel) {
            id = model = attrs;
          } else {
      ...
      fetch: function(options) {
        ...
        return $http.get(LKBaseUrl + this.url(), options);
      },
      ...
      _prepareModel: function(attrs, options) {
        if (attrs instanceof LKRestModel) {return attrs;}
        options = options ? _.clone(options) : {};
  ...
  .factory('LKRestModel', ['$http', 'LKBaseUrl', function ($http, LKBaseUrl) {
    ...
    _.extend(RestModel.prototype, {
      ...
      fetch: function(options) {
        options = options ? _.clone(options) : {};
        if (options.parse === void 0) {options.parse = true;}
        return $http.get(LKBaseUrl.path + this.url(), options);
      },
      ...
      save: function(key, val, options) {
        ...
        if (this.isNew()) {
          var promise = $http.post(LKBaseUrl.path + this.url(), this.toJSON()),
          _this = this;
          ...
          return $http.put(LKBaseUrl.path + this.url(), this.toJSON());
        }
      ...
      destroy: function() {
        ...
        if (!this.isNew()) {
          return $http({method: 'delete', url: LKBaseUrl.path + this.url()});
        }
        return true;
      },
